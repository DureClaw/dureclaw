defmodule HarnessServer.Router do
  ...
  get "/go" do
    ...
    role_env = if role != "", do: "set AGENT_ROLE=#{role}", else: nil
    ...
    script =
      [
        ...
        ~s[curl -fsSL "http://#{server_host}:4000/dist/oah-agent-windows.exe" -o "%TEMP%\\oah-agent.exe"],
        "echo [oah] Starting...",
        ~s["%TEMP%\\oah-agent.exe"]
      ]
  ...
  get "/api/capabilities" do
    ...
    builders =
      ...
      |> Enum.map(fn a ->
        ...
        os =
          caps
          |> Enum.find("unknown", &String.starts_with?(&1, "os:"))
          |> String.replace_prefix("os:", "")
