...
> 一行まとめ: `(1) マーケットプレイス追加 → (2) プラグインインストール → (3) /reload-plugins → (4) /dureteam-status` で Claude Code 内で即利用可能です。

### ステップ 1 — マーケットプレイス追加

Claude Code のプロンプトにそのまま入力してください。
...
> MCP のみ手動登録する場合: `oah setup-mcp` または `curl -fsSL https://dureclaw.baryon.ai/scripts/setup-mcp.sh | bash`

---

### ステップ 3 — プラグインリロード（必須）

インストール後 **必ず一度** 実行してください。スラッシュコマンド・スキル・エージェントが有効化されます。
...
> ロードエラーが 1 件程度表示される場合があります。詳細は `/doctor` で確認できますが、DureClaw 自体の利用には通常影響しません。

---

### ステップ 4 — 最初の実行コマンド（ここから利用可能）

リロード後、以下のいずれかが同じ入口として機能します。

```
/dureteam-status                            ← スラッシュコマンド
"チームの状態を教えて"                    ← 日本語の自然言語
"何人オンライン？" / "show team"          ← 英語混在も OK
```

最初は「チームなし / Phoenix 未接続」と表示されるのが正常です。ローカル 1 台で完結する場合はここまでで十分 — Claude Code 自体がオーケストレーターで、即タスクを実行します。

マルチマシンに拡張する場合はステップ 5 へ進んでください。

---

### ステップ 5 — Phoenix サーバー起動（マルチマシンの場合のみ）

他マシンと連携するには、メッセージバスとなる Phoenix サーバーを 1 台（通常メインのデスクトップ／サーバー）に立ち上げます。
...
自動実行の順序:
1. Phoenix サーバー状態確認 → 未起動なら自動インストール（**Elixir 不要 — Docker または事前ビルド済みバイナリを自動選択**）
2. サーバー IP 自動検出（Tailscale 優先）
3. 現在オンラインのエージェント一覧を表示
4. リモートマシン用ワーカーインストールコマンドを自動生成（macOS / Linux / Windows）
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Port    : 4000
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Tailscale 接続: 100.x.x.x
✅ インストール完了
```

> サーバーはフォアグラウンド（ブロッキング）で実行されます。バックグラウンドで起動するには `nohup … &` または `tmux/screen` を利用してください。

---

### ステップ 6 — ワーカーエージェントのインストール（各リモートマシン）

Phoenix サーバーが起動したら、他のマシンからワーカーを接続します。

**最も簡単な方法** — Claude Code に自然言語で:

```
"ワーカーを追加して"   "tester マシンを接続したい"   "チームに Mac Mini を追加して"
```

Claude がサーバー IP を自動検出し、OS／アーキテクチャ別の **コピー＆実行可能な 1 行コマンド** を案内します。Tailscale がない場合もセットアップを段階的にガイドします。

手動の 1 行コマンドは [分散サブエージェント追加 — OS／アーキテクチャ別 1 行インストール](#分散サブエージェント追加--osアーキテクチャ別-1-行インストール) セクションを参照してください。
...
│   └── marketplace.json
│
├── commands/                   スラッシュコマンド (/setup-team, /dureteam-status)
├── agents/                     エージェント定義（orchestrator など）
├── skills/                     DureClaw オーケストレーションスキル (dureclaw, dureclaw-run)
│
├── packages/
