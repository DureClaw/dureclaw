...
function spawnCompat(opts) {
  if (isBun) {
    const { spawn: spawn2 } = __require("bun");
    return spawn2(opts);
  }
...
async function detectBackend() {
  ...
  try {
    const proc = spawnCompat2({ cmd: [claudeBin, "--version"], stdout: "pipe", stderr: "pipe" });
    await proc.exited;
    if (proc.exitCode === 0) {
...
사람 개입이 필요하면 [ESCALATE: 이유]를 출력하세요.`;
    ...
    try {
      const { text: result, tokenCount: tokens } = await callLLM(userMessage, systemPrompt, role, onProgress);
...
이 전략도 불가능하면 [SKIP]을 붙여라.`;
    try {
      const { text: result, tokenCount: tokens } = await callLLM(userMessage, systemPrompt, state.role, onProgress);
      state.tokenCount += tokens;
      if (result.includes("[SOLVED]")) {
...
async function runPhase1Parallel(task, context, onProgress) {
  ...
  const results = await Promise.all(phase1Roles.map(async (role) => {
...
async function runPhase2Parallel(task, phase1Results, onProgress) {
  ...
  for (const r2 of phase1Results) {
    phase1Context[r2.agentRole.replace("-", "_")] = r2.output.slice(0, 3000);
  }
  const phase2Roles = ["visual-feedback", "executor", "learner-simulator"];
  const results = await Promise.all(phase2Roles.map(async (role) => {
...
[pipeline] iteration ${iteration}/${maxIterations}`);
  ...
  const synthesisResult = await reflectiveAgent(`Synthesize the following analysis results for goal: ${goal}

Phase 1 (Analysis):
${phase1.map((r2) => `## ${r2.agentRole}
${r2.output.slice(0, 2000)}`).join(`
...
4. Provides a go/no-go recommendation for the content`, "orchestrator", {}, onProgress, 3);
...
async function fetchOrCreateWorkKey() {
  ...
  for (let attempt = 1;attempt <= 30; attempt++) {
    ...
    if (attempt % 5 === 0)
      console.log(`[daemon] still waiting... (${attempt * 2}s)`);
  }
...
function handlePhxMessage([msgJoinRef, ref, topic, event, payload]) {
  ...
  switch (event) {
    ...
    case "agent.bye": {
      const p2 = payload;
      if (p2.agent)
        console.log(`[presence] -${p2.agent} left`);
      break;
    }
    case "agent.setRole": {
      const p2 = payload;
      if (p2.to && p2.to !== AGENT_NAME)
        break;
      if (!p2.role)
        break;
...
async function handleTaskAssign(payload) {
  ...
  if (analysisRoles.includes(AGENT_ROLE)) {
    ...
    const result = await reflectiveAgent(payload.instructions, AGENT_ROLE, payload.context ?? {}, onProgress);
...
async function runOpenCode(taskId, payload, signal) {
  const timeout = payload.timeout_ms ?? 300000;
...
function handlePhaseResult(taskId, role, output) {
  ...
  if (_phaseOrch.phase === 1 && _phaseOrch.phase1TaskIds.has(taskId)) {
    ...
    _phaseOrch.phase1Results.forEach((v2, k2) => {
      phase1Partial[k2] = v2.output.slice(0, 3000);
    });
...
function dispatchPhase2() {
  ...
  const phase1Context = {};
  _phaseOrch.phase1Results.forEach((v2, k2) => {
    phase1Context[k2] = v2.output.slice(0, 3000);
  });
